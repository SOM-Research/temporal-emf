...
//	// TODO: Change in final version by short version to save space
	...
	public DirectWriteHbaseResourceEStoreImpl(Resource.Internal resource, Connection connection) throws IOException {
		...
		if (!connection.getAdmin().tableExists(tableName)) {
			...
			propertyFamily.setMinVersions(5);
			propertyFamily.setMaxVersions(10);
	...
	public Object get(InternalEObject object, EStructuralFeature feature, int index) {
		...
		if (feature instanceof EAttribute) {
			return get(klyoEObject, (EAttribute) feature, index);
		} else if (feature instanceof EReference) {
			return get(klyoEObject, (EReference) feature, index);
		} else {
			throw new IllegalArgumentException(feature.toString());
		}
	}
	
	protected Object get(KlyoEObject object, EAttribute eAttribute, int index) {
	...
	protected Object get(KlyoEObject object, EReference eReference, int index) {
	...
	public Object set(InternalEObject object, EStructuralFeature feature, int index, Object value) {
		...
		if (feature instanceof EAttribute) {
			...
			KlyoInternalEObject referencedEObject = KlyoEObjectAdapterFactoryImpl.getAdapter(value, KlyoInternalEObject.class);
	...
	protected Object set(KlyoEObject object, EReference eReference, int index, KlyoInternalEObject referencedObject) {
		...
		try {
			if (!eReference.isMany()) {
				Put put = new Put(Bytes.toBytes(object.klyoId()));
				put.addColumn(PROPERTY_FAMILY, Bytes.toBytes(eReference.getName()), Bytes.toBytes(referencedObject.klyoId()));
				table.put(put);
			} else {
	...
	public boolean isSet(InternalEObject object, EStructuralFeature feature) {
		KlyoEObject klyoEObject = KlyoEObjectAdapterFactoryImpl.getAdapter(object, KlyoEObject.class);
		try {
			Result result= table.get(new Get(Bytes.toBytes(klyoEObject.klyoId())));
	...
	public int size(InternalEObject object, EStructuralFeature feature) {
		KlyoEObject klyoEObject = KlyoEObjectAdapterFactoryImpl.getAdapter(object, KlyoEObject.class);
		String[] array = (String[]) getFromTable(klyoEObject, feature);
	...
	protected void updateContainment(KlyoEObject object, EReference eReference, KlyoEObject referencedObject) {
		if (eReference.isContainment()) {
			try {
				...
				table.checkAndPut(
						Bytes.toBytes(referencedObject.klyoId()), CONTAINMENT_FAMILY, CONTAINER_QUALIFIER, CompareOp.NOT_EQUAL,
						Bytes.toBytes(object.klyoId()), put);
//				table.put(put);
	...
	protected Object getFromTable(KlyoEObject object, EStructuralFeature feature) {
		try {
			Result result = table.get(new Get(Bytes.toBytes(object.klyoId())));
			byte[] value = result.getValue(PROPERTY_FAMILY, Bytes.toBytes(feature.getName()));
			if (!feature.isMany()) {
